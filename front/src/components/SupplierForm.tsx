...
type Props = {
  btnText: string,
  setSuppliersList?: React.Dispatch<React.SetStateAction<ITaskSupplier[]>>,
  suppliersList: ITaskSupplier[],
  closeModal?(): void,
  supplier?: ITaskSupplier | null,
  handleUpdate?(supplier: ITaskSupplier): void
}

const SupplierForm = ({btnText ,setSuppliersList, suppliersList, closeModal, supplier, handleUpdate}: Props) => {
   ...
   const [cidades, setCidades] = useState<{ nome: string }[]>([])
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
      e.preventDefault()
      if(handleUpdate) {
        handleUpdate({...supplier, ...form} as ITaskSupplier)
      } else {
        try {
        const response = await axios.post('http://localhost:3000/suppliers', form, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        })
        setSuppliersList!([...suppliersList, response.data])
        closeModal!()
      } catch (error) {
