...
const Login = ({ setToken }: { setToken: (token: string) => void }) => {
  ...
  return (
    ...
    <form onSubmit={handleSubmit} className='bg-app-lgreen md:w-2/5 p-7 shadow-2xl rounded-2xl h-68 flex flex-col gap-6 text-app-brown text-lg'>
      <div>
        <label htmlFor='email'>E-mail: </label>
        <input id='email' name='email' type='email' onChange={handleChange} required className='w-full p-2 rounded-lg bg-app-white'/>
      </div>
      <div>
        <label htmlFor='password'>Senha: </label>
        <input id='password' name='password' type='password' onChange={handleChange} required className='w-full p-2 rounded-lg bg-app-white'/>
      </div>
